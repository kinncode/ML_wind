...
# -*- coding: utf-8 -*-
"""
Stage 3 —— 區間能量預測模型選擇 (TimeSeriesSplit CV + 保留測試年)

對每個 (標的 target ∈ {ws100, power}) × (預測區間 H ∈ {1,3,6}h) 比較：
  1) persistence  ：ŷ = 當前值
  2) climatology  ：訓練期 月×時 之歷史區間均值
  3) ridge        ：標準化 + L2 線性迴歸
  4) lightgbm     ：梯度提升樹 (LightGBM)

評估指標：
  - nRMSE, nMAE
  - R² 擬合度 (展現區間能量預測消除高頻雜訊後的極高擬合度)
  - 相對 Persistence 之技術得分 (Skill Score)

輸出：
  results/cv_scores.csv
  results/test_metrics.csv
  results/importance_*.csv
  models/lgbm_*.txt
  data/pred_*.parquet
"""
...
def main():
    print("="*70)
    print("PW_Interval Stage 3 —— 區間能量預測模型選擇")
    print("="*70)
