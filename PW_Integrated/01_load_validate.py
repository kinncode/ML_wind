...
輸入：已聚合之 10 分鐘 BSMI 測風塔資料 (BSMI_10min.parquet)
動作：
  1. 獨立跑四重 QC 驗證：
     - 時間軸單調排序與去重
     - 連續 10 分鐘網格對齊
     - 覆蓋率與物理範圍檢查 (風速 0-60 m/s, 氣溫, 氣壓, 濕度, 空氣密度)
     - 感測器凍結/卡死偵測 (連續 >=1h 定值)
     - 四高度風速一致性 (100E/100W 配對差 >3.0 m/s 剔除)
  2. 標記 is_ok，輸出乾淨連續 10 分鐘序列至 data/clean_10min.parquet
  3. 匯出 results/validation_report.txt
"""
...
def main():
    log("="*70)
    log("PW_Integrated Stage 1 —— 載入 + 獨立 QC 驗證")
    log("="*70)
    ...
    # 感測器凍結：WS_100E_mean 連續 >=6 筆 (=1h) 定值
    ...
    # 四高度一致性：100E 與 100W 差過大 (>3.0 m/s)
