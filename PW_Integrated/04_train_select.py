...
# -*- coding: utf-8 -*-
"""
Stage 4 —— 點預測模型選擇 (TimeSeriesSplit CV + 保留測試年)

對每個 (標的 target ∈ {ws100, power}) × (時程 H ∈ {1,3,6}h) 比較：
  1) persistence  ：ŷ = 當前值 (風速 WS_100_mean / 出力 P_now)
  2) climatology  ：訓練期 月×時 之標的歷史均值
  3) ridge        ：標準化 + L2 線性迴歸
  4) lightgbm     ：梯度提升樹 (LightGBM)

驗證：
  - 訓練期 (< TEST_START) 內採 Expanding-window TimeSeriesSplit (4 折) 進行 CV 評估
  - 以整體訓練期模型對保留測試年 (2020-06 起) 評估 nRMSE, nMAE 與相對 Persistence 技術得分
  - 挑選測試年 nRMSE 最低者為最佳模型

輸出：
  results/cv_scores.csv        每折 CV 分數
  results/test_metrics.csv     測試年各模型指標 + 是否最佳
  results/importance_*.csv     最佳 LightGBM 特徵重要度
  models/lgbm_*.txt            儲存之模型
  data/pred_*.parquet          測試年真實值與各模型預測值
"""
...
def main():
    print("="*70)
    print("PW_Integrated Stage 4 —— 點預測模型選擇 (含 XGBoost / LightGBM 對比)")
    print("="*70)
    ...
    xgb_params = dict(n_estimators=350, learning_rate=0.05, max_depth=6,
                      subsample=0.8, colsample_bytree=0.8, reg_lambda=1.0,
                      random_state=C.RANDOM_SEED, n_jobs=-1, tree_method="hist")
    ...
    for target in C.TARGETS:
        for h in C.HORIZONS_H:
            ...
            print(f"\n=== {tag} ===  Train 樣本 {len(tr):,} / Test 樣本 {len(te):,}  (特徵數 {len(fcols)})")
            ...
            xmodel.fit(Xtr[:-nval], ytr[:-nval],
                       eval_set=[(Xtr[-nval:], ytr[-nval:])],
                       verbose=False)
            pred_xgb = xmodel.predict(Xte)
            if target == "power": pred_xgb = np.clip(pred_xgb, 0, 1)
            results["xgboost"] = metrics(yte, pred_xgb, pte)
            ...
            print(f"   TEST nRMSE: " + "  ".join(f"{n}={results[n]['nRMSE']:.4f}" for n in results) + f"  → 最佳={best}")

            # 儲存最佳模型與結果
            gbm.booster_.save_model(os.path.join(C.MODEL_DIR, f"lgbm_{tag}.txt"))
            xmodel.save_model(os.path.join(C.MODEL_DIR, f"xgb_{tag}.json"))
    ...
    pd.DataFrame(cv_rows).to_csv(os.path.join(C.RES_DIR, "cv_scores.csv"), index=False)
    pd.DataFrame(test_rows).to_csv(os.path.join(C.RES_DIR, "test_metrics.csv"), index=False)

    print(f"\n模型訓練與評估完成，指標已寫入 {os.path.join(C.RES_DIR, 'test_metrics.csv')}")
    print("Stage 4 完成。")
